...
const CollectionTypeFormWrapper = ({ allLayoutData, children, slug, id, origin }) => {
  ...
  const { componentsDataStructure, contentTypeDataStructure, data, isLoading, status } = (0,es.useSelector)(crudReducer_selectors);
...
const EditViewDataManagerProvider = ({
  ...
  const { setModifiedDataOnly } = (0,es.useSelector)(crudReducer_selectors);
  const reduxDispatch = (0,es.useDispatch)();
  const toggleNotification = (0,helper_plugin_esm.useNotification)();
...
const EditViewLayoutManager = ({ layout, ...rest }) => {
  const currentLayout = (0,es.useSelector)(selectors/* default */.Z);
  const dispatch = (0,es.useDispatch)();
...
const ModalForm = ({ onMetaChange, onSizeChange }) => {
  ...
  const { schemas } = (0,es.useSelector)((state) => schemasSelector(state), es.shallowEqual);
  const fieldSizes = (0,es.useSelector)(App_selectors/* selectFieldSizes */.Jg);
...
const FieldPicker = ({ layout }) => {
  const dispatch = (0,es.useDispatch)();
  const displayedHeaders = (0,es.useSelector)(selectDisplayedHeaders);
  const { trackUsage } = (0,helper_plugin_esm.useTracking)();
  const { formatMessage } = (0,useIntl/* default */.Z)();
...
const withConnect = (0,es.connect)(mapStateToProps, mapDispatchToProps);
...
const LeftMenu = () => {
  ...
  const { collectionTypeLinks, singleTypeLinks } = (0,es.useSelector)(modelLinksSelector, es.shallowEqual);
...
const useContentManagerInitData = () => {
  const dispatch = (0,es.useDispatch)();
  const toggleNotification = (0,helper_plugin_esm.useNotification)();
  const state = (0,es.useSelector)((0,App_selectors/* selectAppDomain */.Yg)());
